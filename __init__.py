...
def test():
	...
	try:
		...
		if request.method == "POST":
			msg = Message("Minute.tech - Email Verification", sender = "hello@minute.tech", recipients=[session['email']])
...
def homepage():
	...
	if request.method == "POST" and form.validate():
		...
		if int(x) > 0:
			...
			msg = Message("Minute.tech - Question Recieved", sender = "hello@minute.tech", recipients=[email])
			...
			msg = Message("Minute.tech - Email Verification", sender = "hello@minute.tech", recipients=[email])
...
def technician():
	...
	if request.method == "POST" and form.validate():
		...
		if int(x) > 0:
			...
			msg = Message("Minute.tech - Response recieved", sender = "hello@minute.tech", recipients=[email])
...
def fforgot_password():
	try:
		...
		msg = Message("Minute.tech - Forgot Password", sender = "hello@minute.tech", recipients=[f_email])
	...
	try:
		...
		if request.method == "POST" and form.validate():
			...
			if int(x) > 0:
				...
				msg = Message("Minute.tech - Email Verification", sender = "hello@minute.tech", recipients=[techemail])
				link = url_for('techemail_verify', token=token, _external=True)
